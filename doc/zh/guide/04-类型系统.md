...
数字类型是`batScript`内建的自定义类型,表示数字.其中,整数位可以无限大,小数位最多保留32位(溢出截断),有符号,足够绝大多数情况下的使用.
...
可以使用`""`或`''`来表示一个标准字符串.在`""`中`"`需要转义,反之亦然.  
...
set nuums_arr: arr<num> = buildNumWithValue(1, 5); # n[1, 1, 1, 1, 1]
set strs_arr: arr<str> = buildStrWithValue("", 2); # s["", ""]
set bools_arr: arr<bool> = buildBoolWithValue(true, 0); # b[]
```
...
set arr_1: arr<num> = buildNumByIncrease(0, 1, 3); # 0作为第一项, 步进为1, 3个元素: n[0, 1, 2]
```
...
safeGetArrBool(b[true], 1); # 访问第二项.返回Nil
```
