...
```batscript
set var_1: num; # 声明num类型变量 var_1, 默认值为`Nil`
```

在声明时,可以通过`=`为变量赋值:

> 如果变量有赋值,那`batScript`将会自动推导变量的类型,此时变量类型声明可以省略

>> 特别的, 如果是声明一个一维数组, 使用空`[]`构建时必须显式的声明类型

```batscript
set var_1: bool = true; # 声明bool类型变量 var_1, 并赋初始值true
set var_2 = "MyVar"; # 声明str类型变量 var_2, 并赋初始值"MyVar". 类型自动推导为`str`
...
数字类型是`batScript`内建的自定义类型,表示数字.其中,整数位可以无限大,小数位最多保留64位(溢出截断),有符号,足够绝大多数情况下的使用.
...
数字可以进行格式化处理.最常用的是`Num`库提供的系列函数/过程:

```batscript
lib Num;

# 转换整数
set num_1: int = intLimit(3.1415926); # num_1: 3
...
```batscript
set arr_1: arr<num> = []; # 以空数组初始化
```

##### 一维数组的高级构建

常用库`Array`的`build`系列函数/过程提供了一位数组的一些高级构建.例如,以下函数/过程用来构造均为某个值的一位数组:

```batscript
lib Array;

set nuums_arr: arr<num> = buildNumWithValue(1, 5); # [1, 1, 1, 1, 1]
set strs_arr: arr<str> = buildStrWithValue("", 2); # ["", ""]
set bools_arr: arr<bool> = buildBoolWithValue(true, 0); # []
```
...
safeGetArrBool([true], 1); # 访问第二项.返回该类型的缺省值, 即bool的缺省值false
safeGetArrBoolWithDefault([true], 1, true); # 访问第二项.返回设定的缺省值, 即第三个参数true
```
